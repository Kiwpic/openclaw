...
async function runGatewayCommand(opts) {
	...
	const { startGatewayServer } = await startupTrace.measure("cli.server-import", () => withProgress({
		...
		indeterminate: true
	}, async () => import("./server-CiRiKhSp.js")));
	setConsoleTimestampPrefix(true);
	if (devMode) {
		const { ensureDevGatewayConfig } = await import("./dev-PHiI8_Fo.js");
	...
	try {
		...
		if (isGatewayLockError(err)) {
			...
			const { maybeExplainGatewayServiceStop } = await import("./shared-DsQ3XJrr.js");
