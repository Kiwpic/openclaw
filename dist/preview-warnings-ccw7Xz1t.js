...
const channelDoctorModuleLoader = createLazyImportLoader(() => import("./channel-doctor-VSxR915m.js"));
...
async function collectDoctorPreviewNotes(params) {
	...
	const { collectActiveToolSchemaProjectionWarnings } = await import("./active-tool-schema-warnings-BcHUmgR0.js");
	warnings.push(...collectActiveToolSchemaProjectionWarnings({
		cfg: params.cfg,
		env
	}));
	const channelPluginRuntime = hasChannelConfig && hasExplicitChannelPluginBlockerConfig(params.cfg) ? await import("./channel-plugin-blockers-BsVwCgTf.js") : void 0;
	...
	if ((hasPluginConfig || hasChannelConfig) && params.cfg.plugins?.enabled !== false) {
		const { collectStalePluginConfigWarnings, isStalePluginAutoRepairBlocked, scanStalePluginConfig } = await import("./stale-plugin-config-CB2nISDs.js");
	...
	if (hasPluginConfig) {
		const { collectCodexRouteWarnings } = await import("./codex-route-warnings-DlE0s_94.js");
		warnings.push(...collectCodexRouteWarnings({
			cfg: params.cfg,
			env
		}));
		const { collectContextEngineHostCompatibilityWarnings } = await import("./context-engine-host-compat-D8DhMYeg.js");
	...
	if (hasChannelConfig) {
		...
		const { scanEmptyAllowlistPolicyWarnings } = await import("./empty-allowlist-scan-IIK3Igkr.js");
	...
	if (hasToolsBySenderKey(params.cfg)) {
		const { collectLegacyToolsBySenderWarnings, scanLegacyToolsBySenderKeys } = await import("./legacy-tools-by-sender-2f-uqAKE.js");
	...
	const { collectStaleOAuthProfileShadowWarnings, scanStaleOAuthProfileShadows } = await import("./stale-oauth-profile-shadows-BEEcDv8N.js");
