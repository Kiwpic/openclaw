...
const loadOverviewModule = async () => await import("./overview-BT91y_P6.js");
const loadModelsSharedModule = async () => await import("./shared-CQCXxEg3.js");
const loadConfigCliModule = async () => await import("./config-cli-DvdVXsZT.js");
const loadDoctorModule = async () => await import("./doctor-C21_viA2.js");
...
async function runGatewayLifecycle(operation) {
	const lifecycle = await import("./lifecycle-B31_6MwU.js");
	if (operation === "start") {
...
async function executeCrestodianOperation(operation, runtime, opts = {}) {
	...
	if (operation.kind === "plugin-install") {
		...
		await (opts.deps?.runPluginInstall ?? (async (spec, pluginRuntime) => {
			const { runPluginInstallCommand } = await import("./plugins-install-command-Bsy8bjcs.js");
	...
	if (operation.kind === "plugin-uninstall") {
		...
		await (opts.deps?.runPluginUninstall ?? (async (pluginId, pluginRuntime) => {
			const { runPluginUninstallCommand } = await import("./plugins-uninstall-command-CsOIS2tK.js");
	...
	if (operation.kind === "create-agent") {
		...
		const before = await readConfigFileSnapshot();
		const workspace = resolveUserPath(operation.workspace ?? process.cwd());
		await (opts.deps?.runAgentsAdd ?? (await import("./agents.commands.add-B7JqUaeJ.js")).agentsAddCommand)({
	...
	if (operation.kind === "status") {
		...
		const { statusCommand } = await import("./status.command-BkBBuz75.js");
	...
	if (operation.kind === "open-tui") {
		...
		const result = await (opts.deps?.runTui ?? (await import("./tui-BlUg7hP9.js")).runTui)({
			local: true,
			session,
			deliver: false,
			historyLimit: 200
