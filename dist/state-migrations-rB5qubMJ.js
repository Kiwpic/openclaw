...
import { s as writeAcpSessionMetaForMigration } from "./session-meta-M7RjxPxL.js";
...
async function autoMigrateLegacyTaskStateSidecars(params) {
	...
	if (result.changes.length > 0) logger.info(`Auto-migrated legacy task state:\n${result.changes.map((entry) => `- ${entry}`).join("\n")}`);
...
async function autoMigrateLegacyState(params) {
	...
	if (env.OPENCLAW_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim()) {
		...
		return {
			migrated: stateDirResult.migrated || stateSchema.changes.length > 0 || orphanKeys.changes.length > 0 || acpSessionMetadata.changes.length > 0 || pluginStateSidecar.changes.length > 0 || pluginInstallIndex.changes.length > 0 || taskStateSidecars.changes.length > 0 || deliveryQueues.changes.length > 0 || preSessionChannelPlans.changes.length > 0 || pluginPlans.changes.length > 0,
			skipped: true,
			changes,
			warnings
		};
	}
	if (!detected.sessions.hasLegacy && !detected.agentDir.hasLegacy && !detected.channelPlans.hasLegacy && !detected.pluginPlans?.hasLegacy && !detected.pluginStateSidecar.hasLegacy && !detected.pluginInstallIndex.hasLegacy && !detected.stateSchema.hasLegacy && !detected.taskStateSidecars.hasLegacy && !detected.deliveryQueues.hasLegacy) {
		const changes = [
			...stateDirResult.changes,
			...stateSchema.changes,
			...orphanKeys.changes,
