...
import { c as isDiagnosticsTimelineEnabled, s as emitDiagnosticsTimelineEvent } from "./plugin-metadata-snapshot-CYLb0UbD.js";
...
import { A as applyConfigOverrides, c as promoteConfigSnapshotToLastKnownGood, i as getRuntimeConfig, u as readConfigFileSnapshot, v as registerConfigWriteListener } from "./io-CXv-CSA-.js";
...
import { h as setPreRestartDeferralCheck, m as setGatewaySigusr1RestartPolicy } from "./restart-BGjbn7rV.js";
import { n as readGatewayRestartHandoffSync } from "./restart-handoff-B55foNq9.js";
import { f as pinActivePluginChannelRegistry, g as releasePinnedPluginHttpRouteRegistry, h as releasePinnedPluginChannelRegistry, p as pinActivePluginHttpRouteRegistry } from "./runtime-Dew8HF52.js";
...
import { C as stopDiagnosticHeartbeat, S as startDiagnosticHeartbeat } from "./diagnostic-BtIx0uw8.js";
import { i as withDiagnosticPhase } from "./diagnostic-phase-LQperifR.js";
import { o as resolveCronJobsStorePath } from "./store-CurrEjza.js";
import { u as getActiveEmbeddedRunCount } from "./run-state-BYGFyjne.js";
import { l as getTotalQueueSize } from "./command-queue-DRcO14pX.js";
import { n as clearSecretsRuntimeSnapshot, r as getActiveSecretsRuntimeConfigSnapshot } from "./runtime-state-B3MH_XLp.js";
import { s as setFallbackGatewayContextResolver } from "./server-plugins-B4mOlvl3.js";
...
import { a as incrementPresenceVersion, i as getPresenceVersion, n as getHealthCache, o as refreshGatewayHealthSnapshot, r as getHealthVersion } from "./health-state-Dw8JZQvT.js";
import { t as resolveGatewayPluginConfig } from "./runtime-plugin-config-B1AfCVFr.js";
import { n as applyGatewayLaneConcurrency, t as resolveHookClientIpConfig } from "./hook-client-ip-config-C3KNY_us.js";
...
function createLazyGatewayCronState(params) {
	...
	const load = async () => {
		...
		loading ??= import("./server-cron-DMabxqIX.js").then(({ buildGatewayCronService }) => {
			loaded = {
...
function getControlUiModule() {
	controlUiModulePromise ??= import("./control-ui-C18XId_S.js");
	return controlUiModulePromise;
}
function getEmbeddingsHttpModule() {
	embeddingsHttpModulePromise ??= import("./embeddings-http-tQ7XBitV.js");
	return embeddingsHttpModulePromise;
}
function getManagedImageAttachmentsModule() {
	managedImageAttachmentsModulePromise ??= import("./managed-image-attachments-DSMROPfV.js");
	return managedImageAttachmentsModulePromise;
}
function getModelsHttpModule() {
	modelsHttpModulePromise ??= import("./models-http-QQ8bJ1cO.js");
	return modelsHttpModulePromise;
}
function getOpenAiHttpModule() {
	openAiHttpModulePromise ??= import("./openai-http-BZ2glVZk.js");
	return openAiHttpModulePromise;
}
function getOpenResponsesHttpModule() {
	openResponsesHttpModulePromise ??= import("./openresponses-http-CnCWzjyM.js");
	return openResponsesHttpModulePromise;
}
function getSessionHistoryHttpModule() {
	sessionHistoryHttpModulePromise ??= import("./sessions-history-http-Cesf4UhL.js");
	return sessionHistoryHttpModulePromise;
}
function getSessionKillHttpModule() {
	sessionKillHttpModulePromise ??= import("./session-kill-http-NuLQB_eE.js");
	return sessionKillHttpModulePromise;
}
function getToolsInvokeHttpModule() {
	toolsInvokeHttpModulePromise ??= import("./tools-invoke-http-Dy1gHKQ6.js");
	return toolsInvokeHttpModulePromise;
}
function getPluginNodeCapabilityAuthModule() {
	pluginNodeCapabilityAuthModulePromise ??= import("./plugin-node-capability-auth-nkIg0ZGi.js");
	return pluginNodeCapabilityAuthModulePromise;
}
function getHttpAuthUtilsModule() {
	httpAuthUtilsModulePromise ??= import("./http-auth-utils-CDJWYKT9.js");
	return httpAuthUtilsModulePromise;
}
function getPluginRouteRuntimeScopesModule() {
	pluginRouteRuntimeScopesModulePromise ??= import("./plugin-route-runtime-scopes-cJ0FVo9k.js");
...
const loadGatewayPluginsHttpModule = async () => await import("./plugins-http-iKypym8D.js");
	...
	try {
		...
		const handleHooksRequest = async (req, res) => {
			...
			if (!loadedHooksRequestHandler) {
				const { createGatewayHooksRequestHandler } = await import("./hooks-GuKP7_Oh.js");
...
const loadGatewayModelCatalogModule = async () => {
	gatewayModelCatalogModulePromise ??= import("./server-model-catalog-CAiGU_Bo.js");
...
function loadGatewayStartupEarlyModule() {
	gatewayStartupEarlyModulePromise ??= import("./server-startup-early-BkpK9qlx.js");
	return gatewayStartupEarlyModulePromise;
}
function loadGatewayStartupPostAttachModule() {
	gatewayStartupPostAttachModulePromise ??= import("./server-startup-post-attach-D5kC4qXI.js");
...
function getChannelRuntime() {
	cachedChannelRuntimePromise ??= import("./runtime-channel-DjfrrgsA.js").then(({ createRuntimeChannel }) => createRuntimeChannel());
...
const loadGatewayPluginBootstrapModule = async () => {
	gatewayPluginBootstrapModulePromise ??= import("./server-plugin-bootstrap-gNKKCkcA.js");
...
async function stopTaskRegistryMaintenanceOnDemand() {
	const { stopTaskRegistryMaintenance } = await import("./task-registry.maintenance-BaXcffnQ.js");
...
const runDefaultSetupWizard = async (...args) => {
	const { runSetupWizard } = await import("./setup-BYyW9Yri.js");
...
async function startGatewayServer(port = 18789, opts = {}) {
	...
	const startupConfigModulePromise = import("./server-startup-config-DZivQgnt.js");
	let startupPluginsModulePromise = null;
	const loadStartupPluginsModule = () => {
		startupPluginsModulePromise ??= import("./server-startup-plugins-BLy5zq77.js");
	...
	const { createChannelManager } = await import("./server-channels-xjl_AIhj.js");
	...
	try {
		...
		const [{ startGatewayEventSubscriptions }, { startGatewayRuntimeServices }] = await startupTrace.measure("runtime.post-early-imports", () => Promise.all([import("./server-runtime-subscriptions-CxDw5Hmq.js"), import("./server-runtime-startup-services-Cf0Bd_2X.js")]));
		...
		const { execApprovalManager, pluginApprovalManager, extraHandlers, coreGatewayHandlers } = await startupTrace.measure("gateway.handlers", async () => {
			const [{ createGatewayAuxHandlers }, { coreGatewayHandlers: coreGatewayHandlersLocal }] = await Promise.all([import("./server-aux-handlers-DmMLPgNJ.js"), import("./server-methods-DanI8MUU.js")]);
			return {
		...
		const reloadAttachedGatewayPlugins = async (params) => {
			...
			const [{ loadPluginLookUpTable }, { prepareGatewayPluginLoad }, { startPluginServices }] = await Promise.all([
				import("./plugin-lookup-table-BwGk_xPc.js"),
				loadGatewayPluginBootstrapModule(),
				import("./services-B4KJ7iy_.js")
			]);
		...
		const [{ attachGatewayWsHandlers }, { listPluginNodeCapabilities }] = await startupTrace.measure("gateway.ws-imports", () => Promise.all([import("./server-ws-runtime-BpLpenTU.js"), import("./route-capability-9Y_C1THF.js")]));
		...
		const loadScheduledServicesModule = () => {
			scheduledServicesModulePromise ??= import("./server-runtime-services-Cy4Tpueq.js");
		...
		activateScheduledServicesWhenReady();
		const { startManagedGatewayConfigReloader } = await import("./server-reload-handlers-6WY7AJSb.js");
