...
import { t as formatLiteralProviderPrefixedModelRef } from "./model-ref-shared-BQo-ebsO.js";
import "./workspace-B0bmoo98.js";
import { r as resolveManifestProviderAuthChoice } from "./provider-auth-choices-D4Yilxaz.js";
...
import { n as resolveProviderInstallCatalogEntry } from "./provider-install-catalog-CYDMMov5.js";
import { n as applyProviderAuthConfigPatch, t as applyDefaultModel } from "./provider-auth-choice-helpers-C-3OBwob.js";
...
async function applyDefaultModelFromAuthChoice(params) {
	...
	if (!preservesDifferentPrimary) {
		const { CODEX_RUNTIME_PLUGIN_ID, ensureCodexRuntimePluginForModelSelection } = await import("./codex-runtime-plugin-install-BxphkfpF.js");
		...
		const { ensureCopilotRuntimePluginForModelSelection } = await import("./copilot-runtime-plugin-install-wmxzkLuU.js");
...
async function applyAuthChoiceLoadedPluginProvider(params) {
	...
	if (!resolved && installCatalogEntry) {
		const { ensureOnboardingPluginInstalled } = await import("./onboarding-plugin-install-CAR6rD3-.js");
