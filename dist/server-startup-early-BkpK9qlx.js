...
const loadRemoteSkillsRuntimeModule = async () => await import("./remote-BhWWWnqO.js");
...
async function startGatewayEarlyRuntime(params) {
	...
	if (!params.minimalTestGateway) {
		const [{ primeRemoteSkillsCache, setSkillsRemoteRegistry }, taskRegistryMaintenance] = await measureStartup(params.startupTrace, "runtime.early.lazy-runtime-imports", () => Promise.all([loadRemoteSkillsRuntimeModule(), import("./task-registry.maintenance-BaXcffnQ.js")]));
	...
	const skillsChangeUnsub = params.minimalTestGateway ? () => {} : await measureStartup(params.startupTrace, "runtime.early.skills-listener", async () => {
		const [{ registerSkillsChangeListener }, { refreshRemoteBinsForConnectedNodes }] = await Promise.all([import("./refresh-DQzCE5JH.js"), loadRemoteSkillsRuntimeModule()]);
	...
	const startMaintenance = async () => {
		...
		return await measureStartup(params.startupTrace, "post-ready.maintenance", async () => {
			const { startGatewayMaintenanceTimers } = await import("./server-maintenance-Dzjizg5N.js");
