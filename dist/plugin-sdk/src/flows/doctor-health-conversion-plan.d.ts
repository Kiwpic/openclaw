...
export declare const doctorHealthConversionRules: readonly [{
    ...
    readonly target: readonly ["doctor-health-repair-runner", "core/doctor/ui-protocol-freshness"];
    readonly rule: "Delete this bridge after converted checks are registered directly; repair orchestration belongs outside the contribution list. UI freshness is registered for lint/dry-run effects while legacy doctor still owns real repair.";
}, {
    ...
    readonly contributionId: "doctor:disk-space";
    readonly conversion: "terminal-side-effect";
    readonly target: readonly ["doctor-run/disk-space"];
    readonly rule: "Currently emits low/critical free-space warnings via note(); convert to a path-scoped read-only finding (no repair) when the disk-space check gains a structured detector.";
}, {
    ...
    readonly contributionId: "doctor:provider-catalog-projection";
    readonly conversion: "detect-only";
    readonly target: readonly ["core/doctor/provider-catalog-projection"];
    readonly rule: "Validate provider catalog hooks against unified text catalog projection and report malformed plugin catalog rows during doctor.";
}, {
